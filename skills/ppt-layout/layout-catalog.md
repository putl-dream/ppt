...
| PreviewSlide | 排版后视觉摘要 + **640×360 PNG 缩略图**（Electron capturePage） |
| ValidateDeckLayout | 节奏/多样性自动校验 |
| UpdateSlideVariant | 单页 light/dark/hero 背景节奏 |
...
页面需要特殊背景节奏时，在 layout-plan 中使用 `slideVariant` 或最小化的 `designOverride`；不再存在独立背景命令。
...
**BeautifyChart**（P2）：美化已有 chart，或强化明确的 KPI 文本样式；不会从文本臆造图表数据。
...
layout-plan.enhancements 仅接受可原子执行的 `insert-image`。chart/table/icon 必须通过针对具体元素的显式命令处理。
