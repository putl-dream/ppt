...
const originalApplicationDataRoot = process.env[APPLICATION_DATA_ENVIRONMENT_VARIABLE];
...
afterEach(async () => {
  if (originalApplicationDataRoot === undefined) {
    ...
    process.env[APPLICATION_DATA_ENVIRONMENT_VARIABLE] = originalApplicationDataRoot;
  }
  await Promise.all(
