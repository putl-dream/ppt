...
function identitySchema<Name extends string>(name: Name) {
  return z.string().trim().min(1).transform(
    (value) => value as LifecycleIdentity<Name>,
  );
}
...
export type PresentationRevisionNumber =
  z.infer<typeof presentationRevisionNumberSchema>;
...
function validateArtifactStage(
  ...
  if (artifact.stage !== expectedStage) {
    context.addIssue({
      code: "custom",
      message:
        `Artifact kind ${artifact.kind} must be committed at stage ${expectedStage}.`,
      path: ["stage"],
    });
  }
}
...
}).strict().superRefine(validateArtifactStage);
export type ArtifactRevision<T = unknown> =
  Omit<z.infer<typeof artifactRevisionSchema>, "value"> & { value: T };
...
export const pptCapabilityRequestSchema = z.object({
...
export const pptJobParamsSchema = z.object({
...
export const pptJobStateSchema = z.object({
  jobId: pptJobIdSchema,
...
export const pptStageAttemptStatusSchema = z.enum([
  "running",
  "succeeded",
  "failed",
  "cancelled",
]);
export type PptStageAttemptStatus =
  z.infer<typeof pptStageAttemptStatusSchema>;

export const pptStageAttemptSchema = z.object({
  ...
  if (
    attempt.status !== "succeeded"
    && attempt.artifactRevisionId !== undefined
  ) {
    context.addIssue({
      code: "custom",
...
export const pptProposalSchema = z.object({
...
export const pptJobProjectionSchema = z.object({
  jobId: pptJobIdSchema,
  presentationId: presentationIdSchema,
  capability: pptCapabilitySchema,
  requestId: pptCapabilityRequestIdSchema,
