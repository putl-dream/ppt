...
export const TEMPLATE_SUPPORT_LEVELS = ["native", "design-reference", "master-backed"] as const;
...
export const TEMPLATE_CAPABILITIES = ["image", "chart", "table", "diagram", "long-text"] as const;
...
export const uploadedTemplateSourceSchema = z
  .object({
...
export const templateMatchingSchema = z
  .object({
...
export const templateAuthoringGuidanceSchema = z
  .object({
...
export const templateChromeBandSchema = z
  .object({
...
export const templateTitleFrameSchema = z
  .object({
...
export const templateMarginsSchema = z
  .object({
...
export const templateChromeSchema = z
  .object({
...
export const templateTypographyRolesSchema = z
  .object({
...
export const templatePackAssetSchema = z
  .object({
...
export const templateInheritanceSchema = z
  .object({
    colors: z.boolean(),
...
export const templateDescriptorSchema = z
  ...
  .superRefine((descriptor, context) => {
    ...
    if (descriptor.supportLevel === "master-backed" && descriptor.kind === "uploaded") {
      context.addIssue({
        code: "custom",
        path: ["supportLevel"],
        message: "master-backed support is not available; use design-reference instead.",
      });
    }
  });

export const projectTemplatePolicySchema = z
  .object({
    version: z.literal(1),
    ...
    if (policy.mode === "custom") {
      if (!policy.customTemplateId || !policy.customTemplateRevisionId) {
        context.addIssue({
          ...
          message: "mode=custom requires customTemplateId and customTemplateRevisionId.",
        });
      }
    }
  });

export const resolvedTemplateSelectionSchema = z
  .object({
    templateId: z.string().trim().min(1).max(120),
...
export const templateMatchScoreSchema = z
  .object({
    templateId: z.string().trim().min(1).max(120),
    score: z.number(),
...
export const templateCommunicationSignalsSchema = z
  .object({
...
export const templateLibraryIndexSchema = z
  .object({
    version: z.literal(1),
    templates: z
      .array(
...
export function templateRevisionSubPath(templateId: string, revisionId: string): string {
  const safeId = templateId.replace(/[^a-zA-Z0-9._-]+/g, "-");
  const safeRevision = revisionId.replace(/[^a-zA-Z0-9._-]+/g, "-");
  return `${safeId}/${safeRevision}`;
}

export function templateRevisionPath(templateId: string, revisionId: string): string {
