...
export const LEAN_SYSTEM_PROMPT = `你是商业演示文稿架构师。把用户目标一次性转换为 DeckSpec v1。
...
- version 必须是数字 1（不是字符串）；字段名必须是 locale（不要 language），值只能是 zh-CN 或 en-US。
- 每个 source 必须有 id、label、asOf、provenance；asOf 不确定时用 null。
- 每个 slide 必须有 kind、purpose、title、subtitle、items、left、right、steps、metric、chart、sourceRefs；未使用字段分别用空字符串、空数组或 null。不要输出 body、agenda、bullets、comparison、process、closing 等替代字段。
...
function toLeanOutputJsonSchema(): Record<string, unknown> {
  const schema = z.toJSONSchema(leanDeckSpecSchema, {
...
function parseLeanDeckSpecValue(value: unknown): LeanDeckSpec {
  const parsed = leanDeckSpecSchema.safeParse(normalizeLeanDeckSpecInput(value));
  if (!parsed.success) {
...
function parseLeanDeckSpecText(text: string): LeanDeckSpec {
  try {
...
): { spec: LeanDeckSpec; specChars: number } {
...
export class LeanPresentationService {
  constructor(private readonly gateway: AgentModelGateway) {}
  ...
  async createProposal(input: {
    ...
    if (request.length > LEAN_MAX_REQUEST_CHARS) {
      throw new Error(
        `Lean Mode v1 输入上限为 ${LEAN_MAX_REQUEST_CHARS.toLocaleString("zh-CN")} 字符；`
    ...
    const response = await this.gateway.generateText({
      ...
      tools: [{
        ...
        description:
          "Submit the complete DeckSpec v1 exactly once. The arguments are the final response.",
    ...
    const compiled = compileLeanDeckSpec(
      spec,
      input.presentation,
      input.designSystem,
    );
    ...
    return {
      ...
      assumptions: [
        "Lean v1 不执行外部研究、图片检索或多轮 Agent 编排。",
        "版式、坐标、设计系统和元素 ID 均由本地确定性编译器生成。",
