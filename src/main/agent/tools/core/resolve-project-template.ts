...
import {
  TEMPLATE_PACK_PATH,
  templateCommunicationSignalsSchema,
...
async function executeResolve(
  ...
  if (state.pack) {
    if (
      signals.explicitTemplateId
      && signals.explicitTemplateId !== state.pack.templateId
    ) {
      throw new Error(
    ...
    return {
      ...
      template: {
        ...
        description:
          "Uploaded PPTX/POTX design-reference pack. Pages regenerate as SVG.",
      },
      scores: [],
      supportLevelNote:
  ...
  return {
    ...
    supportLevelNote: resolved.template.supportLevel === "design-reference"
