...
/**
 * Backward-compatible helper. Stateful callers should keep one
 * WorkspaceFileService and call service.read() directly.
 */
export async function readWorkspaceText(
  workspaceRoot: string,
  path: string,
): Promise<string> {
  return (await new WorkspaceFileService(workspaceRoot).read(path)).content;
}

/**
 * Backward-compatible helper for creating a new file. Existing files now
 * require a session-scoped read receipt, so use WorkspaceFileService for them.
 */
export async function writeWorkspaceText(
  ...
  return await new WorkspaceFileService(workspaceRoot).write(path, content, options);
}

/**
 * Backward-compatible signature; safe editing requires a session-scoped
 * WorkspaceFileService, therefore a one-shot call will reject existing files
 * with READ_REQUIRED.
 */
export async function editWorkspaceText(
  ...
  return await new WorkspaceFileService(workspaceRoot).edit(
    path,
    oldString,
    newString,
    options,
...
export async function ensureWorkspaceDir(workspaceRoot: string, path: string): Promise<void> {
  const dirPath = await resolveContainedWorkspacePath(workspaceRoot, path);
  const existedBefore = await workspacePathEntryExists(dirPath);
  await mkdir(dirPath, { recursive: true });
  try {
    const resolvedAfterCreate = await resolveContainedWorkspacePath(workspaceRoot, path);
    if (resolvedAfterCreate !== dirPath) {
      throw stalePathError(path, "Directory path identity changed while it was being created.");
    }
    await (await captureWorkspacePathGuard(
      workspaceRoot,
      resolve(dirPath, ".workspace-directory-sentinel"),
      path,
    )).validate();
  } catch (error) {
    if (!existedBefore && error instanceof WorkspaceFileError) {
      throw new Error(
        `Directory creation may have changed the workspace before verification failed: ${path}`,
        { cause: error },
      );
    }
    throw error;
