...
export const commandProposalArtifactValueSchema = z.object({
...
export type CommandProposalArtifactValue =
  z.infer<typeof commandProposalArtifactValueSchema>;

export const presentationRevisionArtifactValueSchema = z.object({
...
export const pptReviewFindingSchema = z.object({
...
export const pptReviewReportSchema = z.object({
...
type CommittedArtifactResult =
  Exclude<ArtifactCommitResult, { type: "conflict" }>
...
export class PresentationLifecycleOrchestrator {
  ...
  private readonly projectionNotificationBatch =
    new AsyncLocalStorage<Map<PresentationId, PptJobProjection>>();
  ...
  assertPresentationSnapshot(
    ...
    const artifact = pointer
      ? this.repository.getArtifactRevision(pointer.revisionId)
      : undefined;
    ...
    if (hashArtifactValue(durable) !== hashArtifactValue(snapshot)) {
      throw new Error(
        "The authoritative Presentation snapshot does not match its durable blob.",
      );
    }
  }
  ...
  async withProjectionNotificationBatch<T>(
    operation: () => Promise<T>,
  ): Promise<T> {
    ...
    try {
      const result = await this.projectionNotificationBatch.run(
        projections,
        operation,
      );
  ...
  beginCapability(input: BeginCapabilityInput): PptJobState {
    return this.withTransaction(
      () => this.beginCapabilityInTransaction(input),
    );
  }

  private beginCapabilityInTransaction(
    input: BeginCapabilityInput,
  ): PptJobState {
    ...
    const existingRequest = input.queryId && existingJob
      ? this.repository.getCapabilityRequestByQuery(
          existingJob.jobId,
          input.queryId,
        )
      : undefined;
    if (existingRequest && existingJob) {
      if (
        ...
        || existingRequest.basePresentationRevisionId
    ...
    if (!existingJob) {
      ...
      if (existingJob.proposalId) {
        const previousProposal = this.repository.getProposal(
          existingJob.proposalId,
        );
        if (previousProposal?.status === "waiting_approval") {
          ...
          if (superseded.type === "conflict") {
            throw new Error(
              `Proposal ${existingJob.proposalId} could not be superseded.`,
            );
          }
        }
      }
    ...
    const intentKind = input.capability === "edit"
      ? "edit_intent"
  ...
  resumeCapability(
    ...
    if (state.status !== "waiting_user") {
      throw new Error(
        `PptJob ${jobId} cannot resume Query ${queryId} while ${state.status}.`,
      );
    }
  ...
  commitArtifact<T>(
    input: CommitArtifactInput<T>,
  ): CommittedArtifactResult {
    return this.withTransaction(
      () => this.commitArtifactInTransaction(input),
    );
  }

  private commitArtifactInTransaction<T>(
    input: CommitArtifactInput<T>,
  ): CommittedArtifactResult {
    ...
    if (existing) {
      ...
      if (
        ...
        throw new Error(
          `Artifact idempotency conflict for ${input.idempotencyKey}.`,
        );
      }
      return {
    ...
    for (const dependency of input.dependencies ?? []) {
      ...
      const authoritativePresentationRebase =
        ...
        && input.artifactId
          === `presentation:${activeState.params.presentationId}`
    ...
    const result = this.repository.commitArtifactRevision(
      revision,
      input.idempotencyKey,
    );
    ...
    const committedArtifacts = [
      ...state.committedArtifacts.filter(
        (item) => item.artifactId !== pointer.artifactId,
      ),
      pointer,
    ];
    const staleArtifacts = [
      ...state.staleArtifacts.filter(
        (item) =>
          ...
          && !stale.staleArtifacts.some(
            (next) => next.revisionId === item.revisionId,
          ),
      ),
  ...
  markArtifactSourceChanged(
    input: MarkArtifactSourceChangedInput,
  ): PptJobState {
    return this.withTransaction(
      () => this.markArtifactSourceChangedInTransaction(input),
    );
  }
  ...
  private markArtifactSourceChangedInTransaction(
    ...
    if (
      ...
      || state.staleArtifacts.some(
        (stale) => stale.revisionId === input.expectedRevisionId,
      )
    ) {
      return state;
    }
    ...
    const staleArtifacts = [
      ...
      ...stale.staleArtifacts.filter(
        (next) =>
          !state.staleArtifacts.some(
            (existing) => existing.revisionId === next.revisionId,
          ),
      ),
    ];
    if (state.proposalId) {
      ...
      if (
        ...
        && staleArtifacts.some(
          (artifact) => artifact.revisionId === proposal.artifactRevisionId,
        )
      ) {
        const superseded = this.repository.resolveProposal({
        ...
        if (superseded.type === "conflict") {
          throw new Error(
            `Proposal ${proposal.proposalId} could not be marked superseded.`,
          );
        }
      }
    }
  ...
  startStageAttempt(input: {
    ...
    if (result.type === "conflict") {
      throw new Error(
        `Stage attempt idempotency conflict for ${input.idempotencyKey}.`,
      );
    }
  ...
  recordProposal(input: {
    ...
    if (state.status !== "running") {
      throw new Error(
        `PptJob ${state.jobId} cannot create a Proposal while ${state.status}.`,
      );
    }
    ...
    if (
      ...
      || state.staleArtifacts.some(
        (artifact) =>
          artifact.revisionId === input.proposalArtifactRevisionId,
      )
    ) {
      throw new Error(
    ...
    if (
      ...
      || (
        ...
        && state.presentationRevisionNumber
          !== input.basePresentationRevisionNumber
      )
    ) {
      throw new Error(
        "Proposal base revision does not match the current Presentation.",
      );
    }
  ...
  recordCommandProposal(input: {
    ...
    const value = commandProposalArtifactValueSchema.parse({
      ...
      gate: {
        ...
        ...(input.gate.warnings
          ...
          : {}),
        ...(input.gate.diff !== undefined
          ? { diff: input.gate.diff }
          : {}),
      },
    });
    ...
    if (existing) {
      ...
      if (
        ...
        || existing.basePresentationRevisionNumber
          !== input.basePresentationRevisionNumber
        || existing.basePresentationRevisionId
          !== input.basePresentationRevisionId
      ...
      if (
        ...
        && (
          state.proposalId !== existing.proposalId
    ...
    const artifactValue = commandProposalArtifactValueSchema.parse(
  ...
  completeReview(input: {
    ...
    if (
      ...
      || !presentationPointer
      || state.staleArtifacts.some(
        (stale) => stale.revisionId === presentationPointer.revisionId,
      )
    ) {
      throw new Error(
    ...
    if (
      ...
      || !hasPresentationRevisionId(
        presentationArtifact.value,
        state.presentationRevisionId,
      )
    ) {
      throw new Error(
    ...
    const idempotencyKey =
      `ppt-review:${state.currentRequest.requestId}:${presentationPointer.revisionId}`;
    const existing = this.repository.getArtifactRevisionByIdempotency(
      state.jobId,
      idempotencyKey,
    );
    ...
    if (existing) {
      ...
      if (state.status !== "running") {
        throw new Error(
          `PptJob ${state.jobId} cannot commit a review while ${state.status}.`,
        );
      }
  ...
  completeExport(input: {
    ...
    return this.withTransaction(() => {
      ...
      if (
        ...
        throw new Error(
          "Export requires the current non-stale PresentationRevision.",
        );
      }
      const presentationArtifact = this.repository.getArtifactRevision(
        presentationPointer.revisionId,
  ...
  completePresentation(input: {
    ...
    if (state.presentationRevisionNumber === input.presentationRevisionNumber) {
      ...
      if (
        ...
        && currentArtifact
        && hashArtifactValue(currentArtifact.value)
          === hashArtifactValue(presentationValue)
      ) {
        return state;
      }
      throw new Error(
  ...
  waitForUser(
    ...
    if (state.status !== "running" && state.status !== "waiting_approval") {
      throw new Error(
        `PptJob ${jobId} cannot wait for the user while ${state.status}.`,
      );
    }
    return this.requireUpdate({
      ...state,
  ...
  private dependencyForProposal(proposalId: ProposalId): ArtifactDependency {
    const proposal = this.repository.getProposal(proposalId);
    if (!proposal) throw new Error(`Unknown proposal ${proposalId}.`);
    const revision = this.repository.getArtifactRevision(
      proposal.artifactRevisionId,
    );
  ...
  private requireCommandProposalArtifact(
    ...
    const artifact = this.repository.getArtifactRevision(
      proposal.artifactRevisionId,
    );
  ...
  private readPresentationBlob(
    ...
    if (
      ...
      throw new Error(
        "PresentationRevision blob has an unsupported media type.",
      );
    }
    ...
    if (bytes.toString("utf8") !== canonicalJson(presentation)) {
      throw new Error(
        "PresentationRevision blob must contain canonical Presentation JSON.",
      );
    }
    return presentation;
  }
  ...
  private emit(state: PptJobState): void {
    const proposal = state.proposalId
      ? this.repository.getProposal(state.proposalId)
      : undefined;
...
function hasPresentationRevisionId(
  value: unknown,
  expected: PresentationRevisionId,
): boolean {
