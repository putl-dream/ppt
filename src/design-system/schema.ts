...
export const hexColorSchema = z.string().regex(
  /^#[0-9a-f]{6}$/i,
  "Expected a six-digit HEX color such as #1d4ed8.",
);
 ...
 */
export const customColorSchemeSchema = z.object({
...
export const colorSchemeSchema = z.union([
  z.enum(COLOR_SCHEMES),
  customColorSchemeSchema,
]);
...
const designSystemV2SourceSchema = z.object({
  version: z.literal(2),
...
export const slideDesignOverrideSchema = z.object({
...
export function resolveDesignTokens(
  ...
  const tokenOverrides = Object.fromEntries(
    DESIGN_TOKEN_KEYS.flatMap((key) => (
      override?.[key] == null ? [] : [[key, override[key]]]
    )),
  );
